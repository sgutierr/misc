...
local function get_public_key(http_client, endpoint)
  ...
  if res.status == 200 then
    ...
    -- sgutierr modification
    -- key = json.public_key
    key = json.keys[1].n
     ngx.log(ngx.INFO, "SGUTIERR: KEY " .. inspect(key))    
  end
...
function _M:authorize(service, client)
  ...
  -- sgutierr@redhat.com apicast impl modification
...
function _M:get_token(service, client)
  ...
  local res = http_client.post(url, ngx.req.get_post_args(), { headers = _M.token_get_headers() })
    --sgutierr modification
    ngx.log(ngx.INFO, "TOKEN_URL POST CALL: " .. inspect(res))
